...
void worksheet1(string namedItem) {
    ...
    if(accountExists) {
      ...
      do {
        cout << "1) View item information\n2) Register a new item or edit an existing one\n3) Sort\n4) Remove an item\n5) Quit\n";
        getline(cin, answerHolder);
        if (answerHolder.length() == 0) {
            answerHolder = "a";
    }
      } while (!validInt(answerHolder) || stoi(answerHolder)> 5 || stoi(answerHolder) < 1);
      boolToInt = 0;
    ...
    switch(boolToInt) {
      case 0: 
        if (answerChoice == 1) {
          ...
          sortInformation(); viewInformation(); break;
        } else if (answerChoice == 4) {
          removeItem(); break;
        } else if (answerChoice == 5) {
